...
import { isValidJson, encrypt, decrypt, storageGet, storageSet, logThis, sendErrorToast, handleError, isInDevMode, tokenToHash } from './helpers.js';


export const STASH_GIST_FILENAME = (isInDevMode()) ? 'dev-stash.json' : 'stash.json';
...
export async function getGitCredsSaved() {
    ...
    if (typeof gitToken !== 'string' || !gitToken.trim()) { return undefined; }
    if (!gistLink?.id) { return undefined; }
    const tokenEncrypted = 'stasher_' + tokenToHash(gitToken);
    return {
        token: gitToken,
        gist: gistLink,
...
export async function getGistContents() {
    ...
    try {
        ...
        if (!req.ok) {
            const errMsg = gist?.message ?? `HTTP ${req.status}`;
            throw new Error(errMsg);
        }
        if (!gist?.files?.[STASH_GIST_FILENAME]) {
            throw new Error(`Gist file "${STASH_GIST_FILENAME}" not found. Reconnect your token in Settings.`);
        }
        ...
        handleError(err.message, `Unable to sync: ${err.message}. Check your GitHub token in Settings.`);
        return undefined;
    }
}
...
export async function setGistContents(data) {
    ...
    try {
        ...
        if (!req.ok) {
            const errMsg = res?.message ?? `HTTP ${req.status}`;
            throw new Error(errMsg);
        }
        ...
        handleError(err.message, `Unable to save to Gist: ${err.message}. Check your GitHub token in Settings.`);
        return;
    }
}
    ...
    try {
        if (stashGist) { // If the stash gist file is already in the users server then use it.
            ...
            const req = await fetch(`https://api.github.com/gists`, {
                ...
                body: JSON.stringify({
                    "description": "Your Stash data is saved here.",
                    "public": false,
                    "files": {
                        [STASH_GIST_FILENAME]: {
                            "content": '0'
            ...
            const res = await req.json();
            if (!req.ok) {
                throw new Error(res?.message ?? `HTTP ${req.status}`);
            }
            if (!res?.id) {
                throw new Error(`Unable to make ${STASH_GIST_FILENAME} gist.`);
            }
        ...
        sendErrorToast(`Gist setup failed: ${err.message}. Check your GitHub token.`);
