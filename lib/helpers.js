...
export async function clearAllNotionPageCodeBlocks(notionToken, blockIds = []) {
    ...
    for (let i = 0; i < blockIds.length; i++) {
        ...
        await delay(1100); // See: https://www.reddit.com/r/Notion/comments/s8uast/error_deleting_all_the_blocks_in_a_page/     
        promises.push(fetch(`https://api.notion.com/v1/blocks/${blockId}`, {
            method: "DELETE",
            headers: getNotionRequestHeaders(notionToken)
        }));
    }
    ...
    for (let c = 0; c < responses.length; c++) {
        ...
        const block = await res.value.json();
        logThis({
            'del': block
        });
        if (block?.object) {
...
export async function saveToNotion(notionToken, notionPageId, contents) {
    if (!navigator.onLine) { return false; }
    const codeBlocks = await getNotionCodeBlocks(notionToken, notionPageId);
    const blockIdsArr = codeBlocks.map((b) => b.notionBlockId);
    if (blockIdsArr.length) {
        clearAllNotionPageCodeBlocks(notionToken, blockIdsArr).then((count) => {
            logThis([`${count} blocks deleted`]);
        });
    }
    const res = await fetch(`https://api.notion.com/v1/blocks/${notionPageId}/children`, {
        method: "PATCH",
        headers: getNotionRequestHeaders(notionToken),
        body: JSON.stringify({
